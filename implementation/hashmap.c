...
Hashmap *newHashmap(char *keyType, char *valueType) {
  ...
  ptrHashmap->array = newDynamicArray();
...
int hashFunctionString(Hashmap *hashmap, void *key) {
  ...
  return sum % hashmap->array->size;
}
int hashFunctionNumber(Hashmap *hashmap, void *key) {
  int keyNumber = *(int *)key;
  return keyNumber % hashmap->array->size;
}
// if hashmap grows i need to reorganize
int hashFunctionUserDefined(Hashmap *hashmap, void *key) {
  // case is a vertex type of a number
  int sum = *(int *)(((Vertex *)key)->value);
  return sum % hashmap->array->size;
}
